...
module.exports = exports = function (options) {
  ...
  pipeline.addStep('Copying volume icon', function (next) {
    if (global.opts['volume-icon']) {
      var finalPath = path.join(global.temporaryMountPath, '.VolumeIcon.icns');
      util.cp(resolvePath(global.opts['volume-icon']), finalPath, function (err) { next(err); });
    } else if (global.opts.icon) {
      var finalPath = path.join(global.temporaryMountPath, '.VolumeIcon.icns');
  ...
  pipeline.addStep('Setting volume icon', function (next) {
    if (global.opts['volume-icon'] || global.opts.icon) {
