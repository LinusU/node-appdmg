...
module.exports = exports = function (source, target, cb) {
  ...
  pipeline.addStep('Looking for app', function (next) {

    var path = resolvePath(global.opts.app);

    fs.exists(path, function (exists) {
      if(exists) {
        next(null);
      } else {
        next(new Error('App not found at: ' + path));
      }
    });
  ...
  pipeline.addStep('Making all the visuals', function (next) {
    ...
    ds.setIconPos(global.appname, global.opts.icons.app[0], global.opts.icons.app[1]);
